...
test("someとfilterの練習", () => {
  ...
  expect(array.some(number => {return number%2 === 0})).toBe(true);
  expect(array1.some(number => {return number%2 === 0})).toBe(false);
  expect(array2.some(number => {return number%2 === 0})).toBe(true);

  expect(array.filter(number => {return number%2 === 0})).toStrictEqual([2,4]);
  expect(array1.filter(number => {return number%2 === 0})).toStrictEqual([]);
  expect(array2.filter(number => {return number%2 === 0})).toStrictEqual([2,4,6,8]);

  expect(array.filter(number => {return number%2 !== 0})).toStrictEqual([1,3,5]);
